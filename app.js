...
inventors.filter(inventors => inventors.year >= 1500 && inventors.year < 1600)
.forEach(inventors => veryOldInventors.push(inventors))
...
sorted.forEach(inventors => sortedByBirthYear.push(inventors))
...
const found = inventors.find ( inventors => inventors.first === "Ada")
if (found) {
    inventorNamedAda.push(found)
}
...
isAdultPresent = devs.some(adult =>adult.year >= 2006)
...
isEveryone19OrOlder = devs.every(devs => devs.year >= 2005)
